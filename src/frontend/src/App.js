...
import { deleteStudent, getAllStudents, getStudentsPage, updateStudent, addNewStudent } from "./Client";
    ...
    switch (selectedMenuKey) {
      case 'dashboard':
        return <>
          ...
          {!fetching && stats.total === 0 && <Empty description="No students yet" />}
          {!fetching && stats.total > 0 && <>
            <Row gutter={[16,16]}>
              <Col xs={24} sm={12} md={8} lg={6}>
                <Card size="small">
                  <Statistic title="Total Students" value={stats.total} />
                </Card>
              </Col>
              <Col xs={24} sm={12} md={8} lg={6}>
                <Card size="small">
                  <Statistic title="Distinct Genders" value={Object.keys(stats.genderCounts).length} />
                </Card>
              </Col>
              <Col xs={24} sm={12} md={8} lg={6}>
                <Card size="small">
            ...
            <Row gutter={[16,16]}>
              <Col xs={24} md={12}>
                <Card size="small" bodyStyle={{ height: 260 }}>
                  <ResponsiveContainer width="100%" height="100%">
                    <PieChart>
                      <Pie data={Object.entries(stats.genderCounts).map(([g,c]) => ({ name:g, value:c, percent: stats.genderPercentages[g] }))} dataKey="value" nameKey="name" outerRadius={80} label={(d) => `${d.name} ${d.percent}%`}>
                        {Object.entries(stats.genderCounts).map(([g], idx) => <Cell key={g} fill={['#1677ff','#13c2c2','#eb2f96','#faad14','#722ed1','#52c41a'][idx % 6]} />)}
                      </Pie>
                      <Legend />
                    </PieChart>
                  </ResponsiveContainer>
              ...
              <Col xs={24} md={12}>
                <Card size="small" bodyStyle={{ height: 260 }}>
                  <ResponsiveContainer width="100%" height="100%">
                    <BarChart data={stats.domains.slice(0,10)}>
                      <XAxis dataKey="domain" hide />
                      <YAxis allowDecimals={false} />
                      <Legend />
                      <Bar dataKey="count" name="Top Domains" fill="#1677ff" />
                    </BarChart>
                  </ResponsiveContainer>
                </Card>
              </Col>
                ...
                <Col key={g} xs={12} sm={8} md={6} lg={4}>
                  <Card size="small" bodyStyle={{ padding: 12 }}>
                    ...
                    <Space direction="vertical" size={4}>
                      ...
                      <Tag color="geekblue" style={{ margin: 0 }}>{stats.genderPercentages[g]}%</Tag>
                    </Space>
                  </Card>
                </Col>
              ))}
